...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'kitty-streets':
				dialogue = CoolUtil.coolTextFile(Paths.txt('kitty-streets/kitty-streetsDialogue'));
			case 'purr-ple':
				dialogue = CoolUtil.coolTextFile(Paths.txt('purr-ple/purr-pleDialogue'));
			case 'X3':
				dialogue = CoolUtil.coolTextFile(Paths.txt('x3/X3dialogue'));
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'kitty-streets':
			{
					defaultCamZoom = 0.8;
					curStage = 'phaseone';
					var bg:FlxSprite = new FlxSprite(-2100, -750).loadGraphic(Paths.image('week7bg'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
			...
			case 'purr-ple' | 'x3':
			{
					defaultCamZoom = 0.5;
					curStage = 'phasetwo';
					var bg:FlxSprite = new FlxSprite(-2100, -750).loadGraphic(Paths.image('week7bg2'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
